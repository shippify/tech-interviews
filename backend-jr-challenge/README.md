...
5. **Armazenar o resultado da correspondência em um arquivo JSON dentro de um bucket S3** com nome único (por exemplo: `matching-result-<timestamp>.json`).
    - Pergunte ao entrevistador sobre as credenciais temporárias da AWS.
    - O nome do bucket é shippify-public-file na região sa-east-1..
6. Retornar:
    - Uma lista de atribuições incluindo **distância em km.**
    - Uma lista de **entregas não atribuídas**.
    - Uma lista de **motoristas não utilizados**.
    - A URL pública do arquivo salvo no S3.
...
```json
{
  ...
  "s3ResultUrl": "https://my-bucket.s3.amazonaws.com/matching-result-1692719123.json"
}
```
...
- As distâncias devem ser calculadas usando a **fórmula de [Haversine](https://community.esri.com/t5/coordinate-reference-systems-blog/distance-on-a-sphere-the-haversine-formula/ba-p/902128) o fórmula [Euclideana](https://www.cuemath.com/euclidean-distance-formula/)**.
...
1. **Fork** do repositório original no GitHub: [**`https://github.com/shippify/tech-interviews/tree/master/backend-jr-challenge`**](https://www.notion.so/Backend-50-mins-24912304bb2e8095aea9ce0a14f55af5?pvs=21)
...
* Uso correto do SDK da AWS para Node.js.
* Lógica de correspondência correta.
